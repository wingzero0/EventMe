...
public class ActivityManager {
	...
	public void updateKeywordsAndTFsToDatabase(HashMap<String, Double> map){
		...
		// http://140.112.29.228/ActivitySuggestion/activityWordHandler.php?op=insert&num=2&activityID=1&keywordID0=1&keywordID1=5&tf0=10&tf1=222
		   try {
			   // DEBUG
				...
				// assess a php page
				String urlParameters = "op=insert&num="+map.size()+"&activityID="+activityIndex;
				List<String> keys = new ArrayList<String>(map.keySet());
				for(int i = 0; i < keys.size();i++){
				   	String key = keys.get(i);
				   	String keyword = "keyword"+i+"="+URLEncoder.encode(key,"UTF-8");
				   	String tf = "tf"+i+"="+URLEncoder.encode(map.get(key).toString(),"UTF-8");
